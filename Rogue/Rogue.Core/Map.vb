...
Namespace Global.Rogue.Core
  ...
  Public NotInheritable Class Map
    ...
    Private ReadOnly PossibilityOfTurning As Integer = 30 'what is the chance of turning during tunnel digging after random number of cells dug
    Private ReadOnly PossibilityOfSecretDoor As Integer = 20
...
#Region "Private Methods"
    ...
    Private Shared Function ConvertDirectionToFace(whatDirection As Direction) As Face
      Dim aReturnValue As Face

      Select Case whatDirection
        Case Direction.Down
          aReturnValue = Face.Bottom
        Case Direction.Left
          aReturnValue = Face.Left
        Case Direction.Right
          aReturnValue = Face.Right
        Case Direction.Up
          aReturnValue = Face.Top
      End Select
    ...
    Private Sub CreateDoors(rm As Room)
      ...
      For i = 1 To count
        ...
        Dim aPossibility = Param.Randomizer.Next(1, 100)
        If aPossibility <= PossibilityOfSecretDoor Then
          rm.Doors.Add(New Door(sx + x + xoffset, sy + y + yoffset, face, True))
        Else
          rm.Doors.Add(New Door(sx + x + xoffset, sy + y + yoffset, face, False))
        End If
    ...
    Private Sub CreateRandomTunnels()
      ...
      For gridPtr As Integer = 1 To 9
        ...
        If isRoomFound = True Then
          Dim aConnectionCount = CreateTunnelsFromRoom(aFromRoom, True)
      ...
      For position = 0 To Rooms.Count - 1
        ...
        Dim aConnectionCount = CreateTunnelsFromRoom(rm, False)
      ...
      For d = 0 To whatRoom.Doors.Count - 1
        ...
        If coord IsNot Nothing Then
          ...
          If coord IsNot Nothing Then
            ...
            foundCellType = Dig(startX, startY, 0, door.Face, whatFacingDoorsOnlyFlag)
    ...
    Private Function CreateUtilityTunnel(startX%, startY%, targetX%, targetY%, whatStopOnTunnelFlag As Boolean) As Integer
      ...
      'Dim diffX As Integer '= cx - targetX 'negative means target is to right and positive means target is to left
      'Dim diffY As Integer '= cy - targetY 'negative means target is to bottom and positive means target is to top
      ...
      Do
        ...
        If aRandomTurnPotential < PossibilityOfTurning AndAlso aStepsInDirectionCtr > 3 Then
          'if random turn then
          ...
          End If
        End If
        'get random number of steps less than max possible so will have a chance to turn
        If aStepsInDirectionCtr > 5 Then
          aStepsInDirectionCtr = Core.Param.Randomizer.Next(1, aStepsInDirectionCtr - 4)
        End If
        ...
        Do
          ...
          ElseIf possible.Contains(secondary) Then
            'secondary may not be the best direction if an obstruction was found
            ...
            End Select
            ' switch them.
            'TODO this creates some unusual behavior 
            '  in that the distance remaining to dig may be further that necessary 
            '  because of the original primary distance.
            '  It is interesting but if not desired, then can comment out switch
            '     and just set previous = secondary
            previous = primary
            primary = secondary
            secondary = previous
            previous = primary
          ...
          If (Me.Tile(cx, cy) = CellType.StructureTunnel AndAlso whatStopOnTunnelFlag = True) OrElse Me.Tile(cx, cy) = CellType.Unknown Then
            'aStepsInDirectionCtr = 0
            tunnelFound = True
            Exit Do
          Else
          ...
          currentFace = ConvertDirectionToFace(previous)
          Dim foundCellType As CellType
          foundCellType = Dig(cx, cy, CellType.StructureTunnel, currentFace, whatStopOnTunnelFlag)
          If Core.Param.IsDebugMode = True Then
            'Console.ReadLine()
          End If

          If IsCellTypeStoppable(foundCellType, whatStopOnTunnelFlag) = True Then ' (foundCellType = CellType.StructureTunnel AndAlso whatStopOnTunnelFlag = True) OrElse foundCellType = CellType.Unknown OrElse foundCellType = CellType.StructureDoor OrElse foundCellType = CellType.SecretHorizontal OrElse foundCellType = CellType.SecretVertical Then
            'aStepsInDirectionCtr = 0
            tunnelFound = True
            cx = targetX
            cy = targetY
    ...
    Private Function Dig(x%, y%, type%, ByVal whatDirection As Face, ByVal whatStopOnTunnelFlag As Boolean) As CellType
      Dim aReturnValue As CellType = CellType.Void 'should we stop after digging this cell?
      ...
      If x = 0 OrElse y = 0 OrElse x = Core.Param.MapWidth - 1 OrElse y = Core.Param.MapHeight - 1 Then
        aReturnValue = aReturnValue
      End If
      'look if we are next to a stopable cell
      'do not look in the direction just came from
      'look up
      If Not whatDirection = Face.Bottom AndAlso y > 0 Then
        aReturnValue = Tile(x, y - 1)
      End If
      'look down
      If Not IsCellTypeStoppable(aReturnValue, whatStopOnTunnelFlag) = True Then
        If Not whatDirection = Face.Top AndAlso y < Core.Param.MapHeight - 2 Then
          aReturnValue = Tile(x, y + 1)
        End If
      End If
      'look left
      If Not IsCellTypeStoppable(aReturnValue, whatStopOnTunnelFlag) = True Then
        If Not whatDirection = Face.Right AndAlso x > 0 Then
          aReturnValue = Tile(x - 1, y)
        End If
      End If
      'look Right
      If Not IsCellTypeStoppable(aReturnValue, whatStopOnTunnelFlag) = True Then
        If Not whatDirection = Face.Left AndAlso x < Core.Param.MapWidth - 2 Then
          aReturnValue = Tile(x + 1, y)
        End If
      End If


      If Core.Param.IsDebugMode = True Then ' Debugger.IsAttached Then
    ...
    Private Function FindNearestDoor(whatRoom As Room, whatDoor As Door, ByVal whatFacingDoorsOnlyFlag As Boolean) As Coordinate
      Dim aReturnValue As Coordinate = Nothing ' New Coordinate(0, 0)
      'Dim aReturnString As String '= ""
      Dim aTargetRoom As New Room 'the room with the door to go to
      Dim aTargetDoor As New Door(0, 0, 0, False) 'the door to go to
        ...
        Else
          ...
          If isDoorFound = False AndAlso whatFacingDoorsOnlyFlag = False Then
            ...
            If Not whatRoom.GridPosition = 5 Then
              ...
              For Each foundRoom As Room In Rooms
                If foundRoom.GridPosition = 5 Then
              ...
              If aTargetRoom.GridPosition > 0 Then
                'go to target room
                ...
                isDoorFound = True
                'above gets some door, but
                'look for a door not yet connected that does not face away from whatroom
                For Each foundDoor As Door In aTargetRoom.Doors
                  If foundDoor.HasConnections = False Then
                    Select Case whatDoor.Face
                      Case Face.Bottom
                        If aTargetRoom.MapGridColumnLocation = whatRoom.MapGridColumnLocation Then
                          If foundDoor.Face = Face.Top OrElse foundDoor.Face = Face.Left OrElse foundDoor.Face = Face.Right Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                          End If
                        End If
                        If isDoorFound = False AndAlso aTargetRoom.MapGridColumnLocation < whatRoom.MapGridColumnLocation Then
                          If foundDoor.Face = Face.Top OrElse foundDoor.Face = Face.Right Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                        ...
                        End If
                        If isDoorFound = False AndAlso aTargetRoom.MapGridColumnLocation > whatRoom.MapGridColumnLocation Then
                          If foundDoor.Face = Face.Top OrElse foundDoor.Face = Face.Left Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                        ...
                        End If
                      Case Face.Left
                        If aTargetRoom.MapGridRowLocation = whatRoom.MapGridRowLocation Then
                          If foundDoor.Face = Face.Right OrElse foundDoor.Face = Face.Bottom OrElse foundDoor.Face = Face.Top Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                          End If
                        End If
                        If isDoorFound = False AndAlso aTargetRoom.MapGridRowLocation < whatRoom.MapGridRowLocation Then
                          If foundDoor.Face = Face.Bottom OrElse foundDoor.Face = Face.Right Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                      ...
                      Case Face.Right
                        If aTargetRoom.MapGridRowLocation = whatRoom.MapGridRowLocation Then
                          If foundDoor.Face = Face.Left OrElse foundDoor.Face = Face.Bottom OrElse foundDoor.Face = Face.Top Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                        ...
                        End If
                        If isDoorFound = False AndAlso aTargetRoom.MapGridRowLocation < whatRoom.MapGridRowLocation Then
                          If foundDoor.Face = Face.Bottom OrElse foundDoor.Face = Face.Left Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
                      ...
                      Case Face.Top
                        If aTargetRoom.MapGridColumnLocation = whatRoom.MapGridColumnLocation Then
                          If foundDoor.Face = Face.Bottom OrElse foundDoor.Face = Face.Left OrElse foundDoor.Face = Face.Right Then
                            isDoorFound = True
                            aTargetDoor = foundDoor
                            aReturnValue = New Coordinate(aTargetDoor.X, aTargetDoor.Y)
                            Exit For
    ...
    ''' </returns>
    Private Shared Function GetRoomNumberFromXY%(x%, y%)
      ...
      Select Case x
        Case 0 To 25
          Select Case y
            Case 0 To 5 : result = 1
            Case 6 To 13 : result = 4
            Case 14 To 20 : result = 7
            Case Else
              Stop
          End Select
        Case 26 To 53
          Select Case y
            Case 0 To 5 : result = 2
            Case 6 To 13 : result = 5
            Case 14 To 20 : result = 8
            Case Else
              Stop
          End Select
        Case 54 To 80
          Select Case y
            Case 0 To 5 : result = 3
            Case 6 To 13 : result = 6
            Case 14 To 20 : result = 9
            Case Else
              Stop
          End Select
        Case Else
    ...
    Private Shared Function IsCellTypeStoppable(whatCellType As CellType, whatStopOnTunnelFlag As Boolean) As Boolean
      Dim aReturnValue As Boolean = False

      Select Case whatCellType
        Case CellType.StructureTunnel
          If whatStopOnTunnelFlag = True Then
            aReturnValue = True
          End If
        Case CellType.Unknown
          If whatStopOnTunnelFlag = False Then
            aReturnValue = True
          End If
        Case CellType.StructureDoor
          aReturnValue = True
          If whatStopOnTunnelFlag = False Then
            aReturnValue = False
          End If
        Case CellType.SecretHorizontal
          aReturnValue = True
          If whatStopOnTunnelFlag = False Then
            aReturnValue = False
          End If
        Case CellType.SecretVertical
          aReturnValue = True
          If whatStopOnTunnelFlag = False Then
            aReturnValue = False
          End If
        ...
        For Each foundLevel As Level In whatDungeon
          ...
          For i = 0 To 20
            ...
            For c = 0 To 79
              ...
              aCurrentData &= Map.MapCellTypeToChar(Core.Tile.TileTypeToCellType(aTile.Type))
      ...
      For y = 0 To Param.MapHeight - 1
        For x = 0 To Param.MapWidth - 1
          ...
          Select Case Tile(x, y)
            ...
            Case CellType.SecretHorizontal : ch = "/"c : fg = ConsoleColor.White
            Case CellType.SecretVertical : ch = "/"c : fg = ConsoleColor.White
          ...
          If Tile(x, y) = CellType.StructureDoor Then
    ...
    ''' <summary>
    ''' Take the data from a recently created random room and put it in the MapData() object used by all the level functions
    ''' </summary>
    ...
    Private Function UpdateMap$(rm As Room)
      ...
      If 1 = 1 Then
        ...
        'now show the doors
        For Each doorData As Door In rm.Doors
          y = doorData.Y
          x = doorData.X
          'Dim ct = doorData.Type

          If doorData.Secret = True Then
            If doorData.Face = Face.Bottom OrElse doorData.Face = Face.Top Then
              MapCellData(y, x) = CellType.SecretHorizontal
            Else
              MapCellData(y, x) = CellType.SecretVertical
            End If
          Else
    ...
    Private Sub VerifyAllRoomsAccessible()
      ...
      Do While aRoomList.Count > 0
        ...
        Dim aCurrentTile As Coordinate 'New Coordinate(0, 0)
        ...
        For Each aDoor As Core.Door In aCurrentRoom.Doors
          ...
          Do While aCount < aMax AndAlso aTileStack.Count > 0
            ...
            Tile(aCurrentTile.X, aCurrentTile.Y) = CellType.Unknown
            If Core.Param.IsDebugMode = True Then
      ...
      For rowPtr As Integer = 0 To Core.Param.MapHeight - 1
        For colPtr As Integer = 0 To Core.Param.MapWidth - 1
          If Me.Tile(colPtr, rowPtr) = CellType.Unknown Then
            Tile(colPtr, rowPtr) = CellType.StructureTunnel
            If Core.Param.IsDebugMode = True Then
...
#End Region
    ...
    '    Dim currentFace As Face
    '    currentFace = ConvertDirectionToFace(direction)
    '    ' Original direction clear...
    '    Dig(seekX, seekY, 2, currentFace, True)
